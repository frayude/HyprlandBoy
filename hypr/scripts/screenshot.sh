...
# # Quick instant mode: full screen + save + copy paste
take_instant_full() {
    # Gunakan grimblast copysave screen (Layar Penuh, Copy & Save)
    grimblast copysave screen "$NAME"
    
    # Grimblast menyimpan file di $HOME, lalu kita pindahkan
    [[ -f "$HOME/$NAME" && -d "$screenshot_folder" && -w "$screenshot_folder" ]] && mv "$HOME/$NAME" "$screenshot_folder/"

    notify-send -t 5000 "Screenshot copied and saved to $screenshot_folder"
}
...
take_instant_area() {
    ...
    # 1. Freeze screen for region selection (Menggunakan hyprpicker)
    hyprpicker -r -z &
    pid_picker=$!
    trap 'kill "$pid_picker" 2>/dev/null' EXIT
    sleep 0.1

    # 2. User selects region; kill picker on cancel
    ...
    # 4a. Capture screenshot dan simpan ke file sementara ($HOME/$NAME)
    grim -g "$region" "$HOME/$NAME" 
    
    # 4b. Ambil screenshot lagi (melalui piping) dan Salin ke clipboard menggunakan wl-copy
    grim -g "$region" - | wl-copy
    
    # 4c. Notifikasi (Menggantikan notifikasi Grimblast)
    notify-send -t 5000 "Screenshot copied and saved to $screenshot_folder"
    
    # 5. Pindahkan file dari $HOME ke folder Screenshots
    [[ -f "$HOME/$NAME" && -d "$screenshot_folder" && -w "$screenshot_folder" ]] && mv "$HOME/$NAME" "$screenshot_folder/"
}
